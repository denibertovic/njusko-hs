...
notify rt es = do
    ...
    where from = Address {addressName = Nothing, addressEmail = "root@localhost"}
          to = [Address {addressName = Nothing,  addressEmail = T.toStrict $ T.pack e} | e <- es]
...
entrypoint args = do
        conn <- open "db.sqlite"
        execute_ conn "CREATE TABLE IF NOT EXISTS links (id INTEGER PRIMARY KEY, url TEXT UNIQUE)"
        existingUrls <- map fromOnly <$> query_ conn "SELECT url FROM links" :: IO [T.Text]
...
insertMultipleEntries c es = forM_ es $ \e -> do
    insertEntry c e `catch` \err -> do
...
insertEntry c e = execute c "INSERT INTO links (url) VALUES (?)" (Only ((T.unpack e) :: String))
